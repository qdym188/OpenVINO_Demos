...
#define CUSTOM_TEST_TYPE "ZeroOut"

class ZeroOutImpl : public InferenceEngine::ILayerExecImpl {
public:
    explicit ZeroOutImpl(const std::shared_ptr<ngraph::Node>& node) : _node(node) {}
    ...
    execute(std::vector<InferenceEngine::Blob::Ptr>& inputs, std::vector<InferenceEngine::Blob::Ptr>& outputs,
        ...
        if (!wasCalled) {
            std::cout << "Running " + std::string(CUSTOM_TEST_TYPE) + " kernel for the first time (next messages won't be printed)"
                      << std::endl;
            wasCalled = true;
        }
...
class ZeroOutOp: public ngraph::op::Op {
public:
    static constexpr ngraph::NodeTypeInfo type_info{CUSTOM_TEST_TYPE, 0};
    const ngraph::NodeTypeInfo& get_type_info() const override { return type_info;  }

    ZeroOutOp() = default;
    explicit ZeroOutOp(const ngraph::Output<ngraph::Node>& arg): Op({arg}) {
    ...
    std::shared_ptr<ngraph::Node> clone_with_new_inputs(const ngraph::OutputVector& new_args) const override {
        ...
        return std::make_shared<ZeroOutOp>(new_args.at(0));
    }
...
constexpr ngraph::NodeTypeInfo ZeroOutOp::type_info;

class InPlaceExtension : public InferenceEngine::IExtension {
public:
    InPlaceExtension() {
        impls[CUSTOM_TEST_TYPE] = [](const std::shared_ptr<ngraph::Node>& node) -> InferenceEngine::ILayerImpl::Ptr {
            return std::make_shared<ZeroOutImpl>(node);
        };
    }
